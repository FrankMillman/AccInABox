...
UNION ALL
    ...
    FROM (
        ...
        WHERE a.deleted_id = ? AND a.tran_date BETWEEN ? AND ?
            AND loc_type_tbl.location_id = ? AND loc_prop_tbl.location_id != ?
            AND code_code_tbl.ledger_row_id = ?
        ) bal
    GROUP BY code_bs_is, code_bs_is_descr, code_bs_is_seq, code_maj, code_maj_descr, code_maj_seq,
        code_int, code_int_descr, code_int_seq, loc_type, loc_type_descr, loc_type_seq, loc_prop, loc_prop_descr, loc_prop_seq
UNION ALL
...
GROUP BY code_bs_is, code_bs_is_descr, code_bs_is_seq, type, code_maj, code_maj_descr, code_maj_seq, type,
    code_int, code_int_descr, code_int_seq, type, start_date, end_date ORDER BY code_bs_is_seq, code_maj_seq, code_int_seq
...
params = [
    ...
    0, datetime.date(2021, 9, 1), datetime.date(2021, 9, 30), 'inc', 'exp', 3, 1, 3, 0, 'PROP', 'TSK',
    datetime.date(2021, 9, 1), datetime.date(2021, 9, 30),
    0, datetime.date(2021, 9, 1), datetime.date(2021, 9, 30), 'PROP', 'TSK', 1,
    datetime.date(2021, 9, 1), datetime.date(2021, 9, 30),
    0, datetime.date(2021, 9, 1), datetime.date(2021, 9, 30), 'PROP', 'TSK', 1
    ]
