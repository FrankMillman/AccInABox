def get_sql(company, conn, bal_date, ledger_id):
    ...
    sql = """
        ...
        WITH temp AS (
            SELECT d.party_id, a.tran_tot_local, ROW_NUMBER() OVER (PARTITION BY
                a.cust_row_id, a.location_row_id, a.function_row_id, a.source_code_id
                ORDER BY a.tran_date DESC) row_num
            FROM {0}.ar_cust_totals a
            JOIN {0}.ar_customers b ON b.row_id = a.cust_row_id
            JOIN {0}.ar_ledger_params c ON c.row_id = b.ledger_row_id
