...
async def setup_ctrl(db_obj, xml):
    ...
    if await gl_codes.getval('ctrl_acc') is not None:
        raise AibError(head='Control Account',
            body=f"'{await gl_codes.getval('gl_code')}' is already a control a/c")
    await gl_codes.setval('ctrl_acc',
        [db_obj.db_table.module_row_id, await db_obj.getval('row_id'), 'bal'])
    await gl_codes.save()
        ...
        if await db_obj.getval('allow_eff_date') is True:
            ...
            if await gl_codes.getval('ctrl_acc') is not None:
                raise AibError(head='Control Account',
                    body=f"'{await gl_codes.getval('gl_code')}' is already a control a/c")
            await gl_codes.setval('ctrl_acc',
                [db_obj.db_table.module_row_id, await db_obj.getval('row_id'), 'uea'])
            await gl_codes.save()
        ...
        if await db_obj.getval('allow_eff_date') is True:
            ...
            if await gl_codes.getval('ctrl_acc') is not None:
                raise AibError(head='Control Account',
                    body=f"'{await gl_codes.getval('gl_code')}' is already a control a/c")
            await gl_codes.setval('ctrl_acc',
                [db_obj.db_table.module_row_id, await db_obj.getval('row_id'), 'uex'])
            await gl_codes.save()
