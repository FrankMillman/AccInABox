    ...
    'dflt_rule'  : (
        '<case>'
          ...
          '<compare test="[[`if`, ``, `cust_row_id>ledger_row_id>open_items`, `is`, `$True`, ``],'
            ...
            '<pyfunc name="custom.artrans_funcs.alloc_oldest" amount_to_alloc="rec_cust"/>'
          '</compare>'
        '</case>'
        ),
    'col_checks' : None,
...
cols.append ({
    ...
    'long_descr' : 'Discount row id (if ar_tran_disc created)',
...
virt.append ({
    ...
    'sql'        : "SELECT row_id FROM {company}.adm_tran_types WHERE tran_type = 'ar_subrec'",
    })
virt.append ({
